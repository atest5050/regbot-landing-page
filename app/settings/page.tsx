...
export default function SettingsPage() {
  ...
  // ── isPro — load from Supabase profiles (same pattern as chat/page.tsx) ────
  const [isPro, setIsPro] = useState(false);
  useEffect(() => {
    if (!user) return;
    const sb = getSb();
    sb.from("profiles").select("is_pro").eq("id", user.id).single()
      .then(({ data }) => { if (data) setIsPro(!!data.is_pro); });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user]);

  // ── Notification settings — persisted to localStorage for now
  // vUnified-platform-fix: added email notifications section
  ...
  // Populate email from auth user when available
  useEffect(() => {
    if (user?.email) {
      setProfile(p => ({ ...p, email: p.email || user!.email! }));
    }
  }, [user]);
  // Load profile from Supabase profiles table when user is available
  useEffect(() => {
    if (!user) return;
    const sb = getSb();
    sb.from("profiles")
  ...
  const handleDeleteAccount = async () => {
    ...
    setDeleting(true);
    try {
      // Supabase admin delete requires a server-side call; here we sign out as MVP.
      // In production, call an API route that uses the service-role key to delete.
      await getSb().auth.signOut();
      router.push("/");
    } finally {
      setDeleting(false);
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    // vUnified-platform-fix: dark class scoped to root div (same pattern as chat/page.tsx shell)
    <div
    ...
    >
      ...
      <div className="shrink-0 flex items-center gap-3 px-4 py-3 border-b border-slate-200 dark:border-slate-700/60 bg-white dark:bg-[#131e2f]">
        ...
        <h1 className="text-[15px] font-semibold text-slate-900 dark:text-white leading-tight">
          Settings
        </h1>
        {isPro && (
      ...
      >
        <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
          ...
          <SectionCard title="Notifications" icon={<Bell className="h-4 w-4" />}>
            ...
            />
            {/* Granular toggles — disabled when master is off */}
          ...
          {/* ──────────────────────────────────────────────────────────────────
               ...
               vUnified-platform-fix: added Stripe billing section             */}
          <SectionCard title="Subscription & Billing" icon={<CreditCard className="h-4 w-4" />}>
            ...
            <div className="px-5 py-4 flex items-center justify-between gap-4">
              ...
              {isPro ? (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 min-h-[44px] px-4 text-sm font-semibold border border-slate-200 dark:border-slate-600/50 text-slate-700 dark:text-slate-300 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-colors pointer-events-auto"
                  style={{ touchAction: "manipulation"  }}
                >
                  Manage
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 min-h-[44px] px-4 text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors pointer-events-auto"
                  style={{ touchAction: "manipulation" }}
                >
            ...
            {/* Pro feature list — shown for Free users */}
            {!isPro && (
            ...
            {/* Payment settings — Stripe portal link */}
            <div className="px-5 py-4 space-y-2.5">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-between min-h-[44px] px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600/50 bg-slate-50 dark:bg-[#0f1823] hover:bg-slate-100 dark:hover:bg-white/5 transition-colors pointer-events-auto"
                style={{ touchAction: "manipulation"  }}
              >
                <span className="text-sm text-slate-700 dark:text-slate-300">Saved payment methods</span>
              ...
              <a
                ...
                style={{ touchAction: "manipulation"  }}
              >
                <span className="text-sm text-slate-700 dark:text-slate-300">Add new card</span>
                <ExternalLink className="h-4 w-4 text-slate-400 shrink-0" />
              </a>
              <a
                href="https://billing.stripe.com/p/login/test_placeholder"
                ...
                style={{ touchAction: "manipulation"  }}
              >
                <span className="text-sm text-slate-700 dark:text-slate-300">Billing history</span>
                <ExternalLink className="h-4 w-4 text-slate-400 shrink-0" />
              </a>
            </div>
          </SectionCard>
