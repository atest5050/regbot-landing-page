...
export interface DocumentAnalysis {
  ...
  /**
   * Compliance risk flags generated by cross-referencing document data with the
   * business checklist. Critical = operating illegally or expired. Warning = expires
   * within 90 days or condition may not be met. Info = posting requirements, etc.
   */
  riskFlags?: Array<{
  ...
  /**
   * Ongoing operational conditions or restrictions explicitly stated in this document
   * (e.g., "Must display license in a conspicuous location", "Limited to 2 employees").
   */
  conditions?: string[];
}
...
export interface UploadedDocument {
   ...
   * Set when uploading a document to a specific requirement card, or when the
   * AI Form Filler auto-attaches a completed form. Used by BusinessProfileView
   * to show the document under its matching recommended form card.
   * Persisted as form_id in the business_documents table (migration 20240011).
   */
  formId?: string;
}
